...
lemma strategy_from_coupleddsim_retains_coupledsim:
  assumes
    ...
    "initial = AttackerNode p0 q0"
    "play \<in> plays_for_strategy (strategy_from_coupleddsim R)"
  shows
...
next
  ...
  assume subassms:
    ...
    "strategy_from_coupleddsim R (n0 # play) # n0 # play
      \<in> plays_for_strategy (strategy_from_coupleddsim R)"
  ...
  proof safe
    ...
    have "\<exists> qii . R p qii \<and> qi \<longmapsto>^~ a qii"
      using n0_def(2) `R pi qi` `coupled_delay_simulation R`
...
proof clarify
  fix n0 play
  assume subassms:
    "n0 # play \<in> plays_for_strategy(strategy_from_coupleddsim R)"
    "cs_game_defender_node n0"
  then obtain pi qi where 
      piqi_def: "hd (play) = AttackerNode pi qi"
        "cs_game_moves (AttackerNode pi qi) n0" "play \<noteq> []"
  ...
  proof safe
    ...
    have "\<exists> qii . R p qii \<and> qi \<longmapsto>^~ a qii"
      using dsn `R pi qi` `coupled_delay_simulation R`
      unfolding coupled_delay_simulation_def delay_simulation_def  by blast
...
proof (clarify)
  ...
  next
    case (2 n0 play)
    hence "\<not> cs_game_defender_node (strategy_from_coupleddsim R (n0 # play))"
      using cs_game_moves_no_step cs_game_defender_node.elims(2) by metis
    hence "\<not>  player1_wins (strategy_from_coupleddsim R (n0 # play) # n0 # play)"
...
proof (rule)
  assume
    "(\<exists>f. player0_winning_strategy f \<and> sound_strategy f)"
  then obtain f where
    "coupled_delay_simulation (\<lambda>p q. \<exists>play\<in>plays_for_strategy f. hd play = AttackerNode p q)"
  ...
  ultimately show "p \<sqsubseteq>cs q"
