...
definition contrasim ::
  \<open>('s \<Rightarrow> 's \<Rightarrow> bool) \<Rightarrow> bool\<close>
where
  \<open>contrasim R \<equiv> \<forall> p q p' A .
...
definition contrasim_strong_step ::
  \<open>('s \<Rightarrow> 's \<Rightarrow> bool) \<Rightarrow> bool\<close>
where
  \<open>contrasim_strong_step R \<equiv> \<forall> p q p' a .
...
lemma contrasim_challenge_strength_step_impl_strong_step:
  assumes
    \<open>contrasim_step R\<close>
  shows
    \<open>contrasim_strong_step R\<close>
...
lemma contrasim_reflexive:
  shows
    \<open>contrasim (\<lambda> p q . p = q)\<close>
  unfolding contrasim_def using step_weak_step_tau by blast

lemma contrasim_union:
  assumes
    \<open>contrasim R1\<close>
    \<open>contrasim R2\<close>
  shows
    \<open>contrasim (\<lambda> p q . R1 p q \<or> R2 p q)\<close>
...
lemma contrasim_implies_coupling:
  assumes
    \<open>contrasim R\<close> \<comment>\<open>actually also is true with 'weaker' @{term contrasim_step}\<close>
    \<open>R p q\<close>
  shows
    \<open>\<exists> q'. q \<longmapsto>*tau q' \<and> R q' p\<close>
proof -
  have \<open>p \<longmapsto>* tau p\<close> using steps.refl by blast
  hence \<open>p \<Rightarrow>^\<tau> p\<close> using tau_tau by blast
  then obtain q' where \<open>q \<Rightarrow>^\<tau> q'\<close> \<open>R q' p\<close>
    using `R p q` `contrasim R` unfolding contrasim_def  by blast
  then moreover have \<open>q \<longmapsto>* tau q'\<close> using tau_tau by blast
...
proof safe
  fix p q p' a
  assume \<open>R p q\<close> \<open>p \<longmapsto>a  p'\<close>
  then obtain q' where q'_def: \<open>q \<Rightarrow>^a q'\<close> \<open>R q' p'\<close>
    using assms(1) unfolding contrasim_strong_step_def by blast
  thus \<open>\<exists>q'. R p' q' \<and> q \<Rightarrow>^a  q'\<close> using assms(2) by blast
next
  fix p q q' a
  assume \<open>R p q\<close> \<open>q \<longmapsto>a  q'\<close>
  hence \<open>R q p\<close> using assms(2) by blast
  then obtain p' where p'_def: \<open>p \<Rightarrow>^a p'\<close> \<open>R p' q'\<close>
    using `q \<longmapsto>a  q'` assms(1) unfolding contrasim_strong_step_def by blast
  thus \<open>\<exists>p'. R p' q' \<and> p \<Rightarrow>^a  p'\<close> using assms(2) by blast
qed
...
lemma contrasim_challenge_strength_does_not_imply:
  fixes p1 q1
  defines
    \<open>R \<equiv> \<lambda> p q . p = p1 \<and> q = q1\<close>
  assumes
    \<open>p1 \<noteq> q1\<close>
    \<open>trans = (\<lambda> p a p' . False)\<close>
  shows
    \<open>contrasim_strong_step R\<close> \<open>\<not>contrasim R\<close>
